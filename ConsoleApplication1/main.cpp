...
clock_t treaTime_Last, treaTime_Now;			//宝物持续时间   计时功能
...
int mainGame();					//主菜单
void initGame(enum mode mode);		//初始化游戏
...
int mainGame()
{
	...
	SetConsoleTextAttribute(g_hConsoleOutput, FOREGROUND_GREEN | FOREGROUND_INTENSITY);				//设置控制台窗口的字体颜色和背景色
	gotoxy(15, 5);
	printf("┏━━━━━━━━━━━━━━┓");
	gotoxy(15, 6);
	printf("┃%2s%s%2s┃", "", "★弹弹乐★", "");
	...
	if (fopen_s(&fMap,"maps\\MapInfo.dat","r"))
	{
		...
		printf("打开地图信息文件发生错误！");
		gotoxy(LEFT + 4, TOP + 4);
		printf("请查询MapInfo.dat文件是否存在！！！");
		exit(0);											//表示进程非正常退出
	}

	while (fgets(filename[level_Max], 20, fMap) != NULL)			//读取指向的流中n个字符的信息到存储地址，成功返回存储区地址
	{
	...
	do
	{
		ch = _getch();		//读取一个字符，转换为ASCII码，但不显示再屏幕上 不用输入enter
		...
		case 'S':
		case '2':
		case 80:	//下	getch函数在读取一个功能键或者箭头（方向）键盘时，函数会返回两次，第一次调用返回0或者0xE0，第二次调用返回实际的键值。
			if (index == 0)
			{
				index = 1;
				SetConsoleTextAttribute(g_hConsoleOutput, 0x0F);
...
int main()
{
	...
	SetConsoleTitleA("~~~弹弹弹~~~");
	char filename[1000];
	_getcwd(filename, sizeof(filename));					//获取当前路径
	...
	char error[100] = { 0 };
	MCIERROR mciError;
	mciError = mciSendString("open filename alias music", error, 100, NULL);			//打开
	mciError = mciSendString("play music repeat", error, 100, NULL);				//播放
...
void creatMap()
{
	...
	do
	{
		...
		if (ch == 13)
		{
			...
			printf("你确定你保存好了吗？ <按ENTER键确认，按任意键继续编辑");
			_getch();
			if (ch == 13)
			{
				gotoxy(LEFT + 5, BUTTON - 3);
				printf("给你心爱的地图取一个好听的名字吧！--> ");
...
void helpGame()
{
	...
	system("pause>nul");						//pause 就是暂停命令  不显示“请按任意键继续. . .”这些字
